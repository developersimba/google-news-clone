...
<div className="grid grid-cols-2 p-5 rounded-2xl bg-white w-8/12">
<a href={props?.news[7]?.url}><div className="w-72">
    <h1 className="text-blue-600 text-xl">Top Stories</h1>
    <hr className="mt-3"/>
      <img src={props?.news[7]?.urlToImage} className="w-72 h-44 rounded-xl" />
      <h1 className="font-bold text-sm">{props?.news[7]?.source?.name.toUpperCase()}</h1>
      <h1 className="text-xl w-72 hover:underline ">{props?.news[7]?.title}</h1>
    </div>
    </a>
    <div className="w-60">
    ...
    <div className="bg-white w-3/12 rounded-2xl ml-3 p-5">
      <h1 className="text-cyan-800 text-2xl">Local news</h1>
      <hr className="mt-3"/>
      <a href={props?.news[4]?.url}><div className="flex items-center">
        <div className="mt-3" >
